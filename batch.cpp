...
void Initialize(){
	...
	std::ifstream is("Input//ObservationSequence//Observation.txt");
	char c;
	while (is.get(c)) {
		if((c-'0' >= 0) && (c-'0' <= 9))T++;
	}	
	...
	observation = new int[T];
	T=0;
	std::ifstream inp("Input//ObservationSequence//Observation.txt");
	while (inp.get(c)) {
	...
	for(int i=0;i<N;i++)
		...
		for(int i=0;i<N;i++){
			...
			for(int j=0;j<M;j++)
				if(checkB[j]==1){					
					//Split from largest
					bNew[i][j] = bNew[i][index]*(1.000000e-20);
...
//write values of following into the file : a,b,pi
...
for(int i=0;i<N;i++)output<<piNew[i]<<",";
